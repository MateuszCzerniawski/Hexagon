...
void Game::clearSelection() {
    for(Board::Field* field : board->getData())
        if(field->getWhoHere()=='n' || field->getWhoHere()=='d') field->setWhoHere('O');
}
...
void Game::reactToClick(int which){
    ...
    switch (this->turn) {
        ...
        case PLAYER1:
            ...
            if (!havePosMove()) close();
            if (enableRobot) {
                display->refresh(deduceMode());
                std::this_thread::sleep_for(std::chrono::milliseconds(350)); //sleep dla estetyki
                if (turn == PLAYER2) robot->makeMove();
                if (!havePosMove()) close();
                if (turn == PLAYER2) this->turn = PLAYER1;
            }
        }else{
                if(which==62) save();
                backToMenu();
